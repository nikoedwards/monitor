...
- 归档会为常见注册/营销弹窗的关闭按钮和 Esc 键提供安全兜底；若克隆 DOM 丢失原事件监听，仍可隐藏弹窗并恢复页面滚动。服务启动时也会自动升级 Volume 中已有的 HTML 归档，无需重新截图。
...
- Railway Volume 仍需挂载到 `/app/data`。每次完整快照会新增一个 PNG 和一个 HTML 文件，HTML 默认最多内嵌约 30 MB 资源，长期运行时应配置容量监控和保留策略。
