...
- Chromium 若返回 `local_rate_limited`、HTTP 429/5xx 等限流或错误占位页，会更换页面并分段重试；直接导航持续受限时，会使用后端已成功取得的真实 HTML 在原站 URL 下重新渲染。两种方式都失败时不会写入截图、归档或变化分析，也不会推进上次成功截图时间。任务会按 10、30、60、180 分钟自动补拍，连续失败后每 6 小时低频恢复重试；网页截图使用独立调度线程每分钟检查，不会再被其他耗时采集阻塞，监控卡片会显示连续失败次数和下次补拍倒计时。
