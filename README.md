...
- Chromium 若返回 `local_rate_limited`、HTTP 429/5xx 等限流或错误占位页，会更换页面并分段重试；直接导航持续受限时，会使用后端已成功取得的真实 HTML 在原站 URL 下重新渲染。两种方式都失败时才停止，且不会写入截图、归档或变化分析，也不会推进上次成功截图时间；监控卡片会显示失败原因，后台后续仍会继续重试。
